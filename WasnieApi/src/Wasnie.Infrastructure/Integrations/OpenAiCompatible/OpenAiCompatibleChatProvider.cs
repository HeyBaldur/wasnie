    ...
    : IChatCompletionProvider
{
    ...
    /// Null leaves the provider default (1.0) in force. Pass 0 for a CLASSIFIER — see the note on the
    /// dispatcher's payload in <see cref="SelectToolAsync"/> for what sampling on a classifier cost.
    /// </param>
        ...
        IReadOnlyList<ChatMessage> messages, CancellationToken cancellationToken)
    {
        ...
        // ★ THE SECTION ROUTER IS A CLASSIFIER TOO, and it decides which documentation the answering
        // model is shown. Left sampling, the same question reads different sources on different turns,
        // which is the same inconsistency one layer up. Pinned for the same reason as the dispatcher.
        using var request = BuildRequest(
            messages, Settings.Model, stream: false, jsonObject: true, temperature: 0);
        ...
        CancellationToken cancellationToken)
    {
        ...
        //
        // ★★ TEMPERATURE 0, AND THIS IS THE FIX FOR AN OBSERVED DEFECT (KAN-58). No temperature was
        // sent on ANY call in this file, so every one ran at the provider's default of 1.0 — including
        // this one, whose entire job is to pick one label from a fixed set. Runtime verification found
        // exactly what that predicts: the same input, a bare payee name, produced three different
        // behaviours across three attempts — the clarify form once, a greeting once, and a prose
        // question once. Sampling on a CLASSIFIER is not creativity, it is a coin flip about which
        // function the user's question reaches.
        //
        // ★ THE GENERATING CALL IS DELIBERATELY LEFT ALONE. That one writes prose a person reads, its
        // variety is not a defect, and changing how the assistant writes is not what this ticket asked
        // for. Only the two classifiers are pinned.
        var payload = new GroqRequest(
    ...
    /// <param name="Temperature">
    /// Sampling temperature. NULL means "do not send it", which leaves the provider's own default of
    /// 1.0 in force — and that is exactly what was happening on every call in this file (KAN-58).
    /// </param>
