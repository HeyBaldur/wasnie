...
public sealed class BulkDeactivateAssignmentsHandler(
    ...
    IAuditService audit)
    : IRequestHandler<BulkDeactivateAssignmentsCommand, Result<BulkAssignmentOperationResult>>
{
    public async Task<Result<BulkAssignmentOperationResult>> Handle(
        BulkDeactivateAssignmentsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (affected > 0 && assignments.Count > 0)
        {
            await audit.LogAsync(new AuditEntry(
                ...
                Action: AuditActions.AssignmentBulkDeactivated,
