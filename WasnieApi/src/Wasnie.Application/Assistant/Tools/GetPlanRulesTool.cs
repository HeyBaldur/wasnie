...
public sealed class GetPlanRulesTool(ISender sender, ILogger<GetPlanRulesTool> logger) : IAssistantTool
{
    ...
    private static PlanRules Describe(
        ...
        new(
            ...
            Rules: plan.Rules.Select(r => DescribeRule(r, plan.Currency)).ToList());

    private static PlanRule DescribeRule(RuleDto rule, string planCurrency)
    {
    ...
    private enum TriggerToken { Unconditional }

    private enum FieldStatusToken { Recognised, UnknownFieldRuleNeverMatches }

    private enum ModifierSemanticToken { MultipliesCommissionByFactor }

    private enum ModifierConditionToken { Unconditional, ConditionsIgnoredModifierAlwaysApplies }

    private enum LimitEnforcementToken
    {
    ...
    private sealed record PlanRule(
        string RuleName,
        int SortOrder,
    ...
    private sealed record TriggerConditions(
        string LogicalOperator, IReadOnlyList<TriggerCondition> Conditions);

    private sealed record TriggerCondition(
        string Field,
        string Operator,
        string ValueType,
        string? Value,
    ...
    private sealed record RateTableDescription(
        string Type,
    ...
    private sealed record AmountTier(decimal FromAmount, decimal? ToAmount, decimal RawRate);

    private sealed record AttainmentBracket(
        decimal FromAttainmentFraction, decimal? ToAttainmentFraction, decimal RawRate);

    private sealed record ModifierDescription(
        string Type, decimal Factor, string SemanticBehavior, string ConditionHandling);

    private sealed record LimitDescription(
