...
TODO trabajo, no solo al que toca las secciones de arriba. Leela antes de empezar.

**El trabajo se gestiona en JIRA.** Cada tarea, bug o incidente vive en un ticket KAN.
El ticket es la fuente de verdad de qué hay que hacer y en qué estado está. NO se usan
`PROJECT_STATUS.md` ni `SESSION_LOG.md` — están obsoletos. El registro de qué se hizo
va en los COMENTARIOS del ticket (ver §F).

**Authority order (when in conflict):**
`docs/ARCHITECTURE.md` > `docs/Wasnie_Product_Master_Specification.md` >
`WasnieUi/DESIGN_SYSTEM.md` > el ticket de Jira > everything else. Read the relevant
full doc before non-trivial work; this file is a reminder, not a substitute.
...
## 2. Docs de arquitectura viven OUTSIDE the code projects

`ARCHITECTURE.md`, `Wasnie_Product_Master_Specification.md` y demás docs de referencia
están en `<repo>/docs/` — ONE LEVEL ABOVE `WasnieApi/` and `WasnieUi/`. Search there,
not only under the project you're working in.

El estado del trabajo y el registro de sesión NO viven en archivos: viven en Jira. No
hay `PROJECT_STATUS.md` ni `SESSION_LOG.md` que actualizar. Cada WI arranca leyendo su
ticket KAN y termina comentando en él (ver §F).
...
7. El ticket KAN comentado con el resultado (ver §F). No hay archivos de estado que
   actualizar.
...
## 8. Reglas de desarrollo — destiladas de defectos reales
...
El ticket KAN es la fuente de verdad. Ante cualquier tarea:
