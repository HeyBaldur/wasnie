...
**Last updated:** 2026-06-09 — Payouts refinement complete (WI-CALC-A.5.1–A.5.6) + Pay Run model design approved (docs/Pay_Run_Model.md). Smoke test full payout flow (calculate → approve → bulk-approve → mark-paid → audit → idempotency): all green. Money.Zero hardcoded-USD bug fixed. CurrencyFormatPipe global 2-decimal fix. Excel export (payouts list). Next: WI-CALC-A.6 (Pay Run implementation). Known pending: ~11 pre-existing red tests (10 frontend: ProcessPendingComponent×5, TransactionsListComponent×5; 1 backend: AssignmentsEndpointsTests); "This month" filter boundary shows 1st→today instead of 1st→last-day-of-month; legacy $0/USD payouts in dev tenant need recalculation (fix only affects future payouts).
...
**Updated by:** Rodolfo Calvo (post-payouts-refinement + Pay Run design session, 2026-06-09)
...
**Right now we are:** End-of-day 2026-06-09. WI-CALC-A.5.1–A.5.6 Done — full Payouts refinement + Pay Run design session closed. Manual smoke test of complete payout flow in browser: green across all paths. Pay Run model (A.6) design approved; docs/Pay_Run_Model.md is the reference. Recommended next step before A.6: optional WI to clean ~11 pre-existing red tests (10 frontend + 1 backend) so new regressions are visible. Then WI-CALC-A.6 (Pay Run implementation, start with Step 0 read-only + reconciliation against Product Master Spec). Known pending bugs: (a) "This month" filter shows 1st→today instead of 1st→last-day; (b) legacy $0/USD payouts in dev tenant need period recalculation.
...
- **WI-CALC-A.6 ⏳ NEXT** — Pay Run model implementation (design approved docs/Pay_Run_Model.md, Decision #66)
...
**Status:** Pending. Design approved 2026-06-09. Prerequisite: Step 0 read-only diagnosis + reconciliation against `docs/Wasnie_Product_Master_Specification.md` before writing any code.

**Design reference:** `docs/Pay_Run_Model.md` (approved 2026-06-09, 6 closed decisions — see Decision #66).

**Core scope:**
- New `PayRun` domain entity (period, status: Draft→Approved→Paid, `PayeeCount`, `PaidPayeeCount`, multi-currency roll-ups).
- Approve and Pay actions at PayRun level; drill-down to individual payouts.
- Reopen: Approved→Draft for full run only; individual payouts cannot be reopened independently.
- Zero-amount payouts generated but hidden by default (toggle).
- Clawbacks: out of scope for V1 (separate WI post-A.6).
- UI: master-list → PayRun-detail pages (not expandable tree — Decision #72).
- Start with Step 0 → reconcile decisions against Product Master Spec → scoped implementation plan → code.
