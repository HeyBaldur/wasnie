...
**Last updated:** 2026-06-10 — A.6 Fase 6 done (Filters + Excel Export on Pay Run list & detail). Backend: `PayoutFilterQuery` extended with `PayRunId`, `AmountMin`, `AmountMax`; `ListPayoutsHandler.BuildQuery()` now static internal (shared by list, export, and detail handlers); `GetPayRunByIdHandler` maps `PayRunPayoutsDetailFilter` → `PayoutFilterQuery`; `ExportPayRunsHandler` + `PayRunExcelExportService` (dynamic currency columns, Pattern B); `ExportPayoutsHandler` gains `PayRunId` filter; `PayRunsController.Export` endpoint. Frontend: `PayRunPayoutsDetailFilter` model; `PayRunsApiService` updated (`getById` accepts rich filter, `exportPayRuns`, `exportRunPayouts`); `PayRunsStore` + `PayRunDetailStore` extended with `toExportParams()` race guard + `activeFilterCount`; list page: Export to Excel button (`Payouts.Export` gate); detail page: collapsible filter bar (status, period from/to, amountMin/max, payee+plan chips) + Export to Excel button; EN/ES/PL i18n complete. Tests: 43/43 frontend unit pass; `PayRunExportTests.cs` added (11 integration tests). `ng build` clean. Next: commit + deploy.
...
**Updated by:** Rodolfo Calvo (A.5.1–A.5.6 + export race fix + Pay Run design + A.6 Fases 1–4, 2026-06-09)
...
**Right now we are:** End-of-day 2026-06-09. WI-CALC-A.6 Pay Run — Fases 1–4 DONE (Domain + Migration + Engine + 20 integration tests). Totals: 387 unit + 134 integration = 521 (518 pass; 1 pre-existing `AssignmentsEndpointsTests` failure; 2 skip). Build clean. **Fase 5 UI is NEXT**: `PayRunListComponent` at `/pay-runs`, `PayRunDetailComponent` at `/pay-runs/:id`, two new stores, run-level action modals (Approve/Reopen reversible, MarkPaid irreversible + 5 mandatory elements), sidebar `/payouts`→`/pay-runs`, i18n EN+ES+PL. Avoid the `_lastLoadedFilter` race (A.5.6 lesson) in new stores. Known pending bugs: (a) "This month" filter shows 1st→today instead of 1st→last-day-of-month; (b) Excel export shows raw DB 4-decimal values instead of rounding to currency display precision; (c) potential `_lastLoadedFilter` race latent in `CreditsStore` + `TransactionsStore` — audit alongside Fase 5; (d) legacy $0/USD payouts in dev tenant need recalculation; (e) ~11 pre-existing red tests (10 frontend: ProcessPendingComponent×5, TransactionsListComponent×5; 1 backend: AssignmentsEndpointsTests) — fix before A.6 Fase 5 to keep regressions visible.
...
- **Fase 5 UI:** PENDING (next session).
...
- **WI-CALC-A.6 🔄 IN PROGRESS** — Pay Run: Domain + Migration + Engine + 20 integration tests DONE (Fases 1–4). Fase 5 UI pending next session.
...
### WI-CALC-A.6 — Pay Run model implementation ⏳ NEXT

**Status:** IN PROGRESS — Fases 1–4 DONE (2026-06-09). Fase 5 UI pending (next session).
...
- **Integration Tests (Fase 4):** 20 tests (idempotency ×4, state machine valid/invalid ×6, roll-ups/anti-Cartesian ×3, multi-tenant ×2, permission gates ×5). Totals: 387 unit + 134 integration = **521** (518 pass; 1 pre-existing unrelated failure; 2 skip). Build clean.

**Pending (Fase 5 UI — NEXT SESSION):**
- `PayRunListComponent` at `/pay-runs` (replace `/payouts` in sidebar)
- `PayRunDetailComponent` at `/pay-runs/:id` with embedded payout sub-list
- `PayRunsStore` + `PayRunDetailStore` (apply A.5.6 `_lastLoadedFilter` lesson to new stores)
- Run-level action modals: Approve + Reopen (reversible — no irreversibility warning per Decision #69), MarkPaid (irreversible — 5 mandatory elements)
- Full i18n EN+ES+PL; RBAC gating via `*hasPermission`

**Post-A.6 roadmap (deferred — separate WIs):**
- Aggregated payroll export by run (for payroll system integration)
- Email notification on run close: infra Resend (account already created) → export attached → `/admin` settings for recipient list → tier limits Growth 2 / Scale 4 / Enterprise N via `TierLimitChecker` → trigger on PayRun Paid transition. GDPR concern: salary data by email requires strong recipient validation.
