...
/**
 * What a screen hosting the rule definition has to tell it.
 *
 * ★★ THE ONLY THINGS THAT DIFFER BETWEEN THE TWO HOSTS. The Plans rule page and the guided tour's
 * sandbox edit the same definition against the same engine; they differ in WHICH plan the rule
 * belongs to, what currency that plan is in, and where a simulation is sent. Everything else — the
 * fields, the ladders, the warnings, the builder — is one implementation, so a change to what a rule
 * can express reaches both screens at once instead of leaving the tour teaching last month's form.
 */
export interface RuleDefinitionFormOptions {
  /** The plan the rule belongs to. Read when the definition is built, never cached. */
  planId: () => string;
  currency: Signal<string>;
  readOnly: Signal<boolean>;
  /** Where the definition on screen is simulated. The Plans page and the sandbox send it to different plans. */
  simulate: (request: SimulateRuleRequest) => Observable<RuleSimulation>;
}

/** Built inside an injection context — a component field initialiser. */
export function createRuleDefinitionForm(options: RuleDefinitionFormOptions): RuleDefinitionForm {
...
/**
 * A rule's definition — the form, everything it knows about itself, and the simulator beside it.
 *
 * ★★ A MODEL, NOT A COMPONENT, AND THE SPLIT IS THE POINT. It used to live inside the Plans rule
 * page, and the guided tour grew a second, hand-written copy of the same fields. The copy drifted
 * within a week: its cap and floor travelled in a shape the backend does not read, and its
 * attainment ladder sent bounds under names the engine ignores, so both arrived as zeros with no
 * error anywhere. One definition, rendered by `app-rule-definition-fields` and `app-rule-preview`,
 * is what keeps the two screens from telling two different stories about the same rule.
 */
export class RuleDefinitionForm {
  ...
  readonly planCurrency: Signal<string>;
  ...
  readonly form: RuleForm;
  readonly formValue: Signal<RuleForm['value']>;

  /**
   * The id of the modifier the loaded rule already had, so an edit keeps it instead of minting a new
   * one on every save. Null when creating, or when the rule never had a modifier.
   */
  ...
  constructor(
    ...
    this.planCurrency = options.currency;

    this.form = buildRuleForm(this.fb);
  ...
  /**
   * Loads the two catalogs the trigger section is built from.
   *
   * The field catalog drives the picker, the operator lists and the value-type. Without it the form
   * has nothing valid to offer, so failures leave the list empty rather than falling back to a
   * hardcoded copy that could drift from the engine.
   *
   * ★ BOTH ARE THE TENANT'S REAL CATALOGS, IN THE SANDBOX TOO. They are read-only, and what a rule
   * tried in the sandbox is for is to be promoted: a trigger written against the real categories is
   * the one that will match once it pays real money.
   */
  loadCatalogs(): void {
  ...
  /** A new rule: numbered after the ones the plan already has, with one tier ready to fill in. */
  prepareForCreate(sortOrder: number): void {
    this.form.controls.sortOrder.setValue(sortOrder, { emitEvent: false });
    this._addTier();
  }
  ...
  patchFromRule(rule: Rule): void {
    ...
    this.modifierId = rule.modifier?.id ?? null;
   ...
   * simulator matters most. A rule belonging to an active plan is read-only, so the page calls
   * `form.disable()`; a disabled FormGroup has status DISABLED, and `valid` is `status === VALID`, so
   * **`form.valid` is false for every read-only rule** no matter how complete it is. The one place
   * you cannot edit a rule to find out what it pays was the one place the calculator refused to run.
   *
  ...
  /**
   * The simulator's half of the same contract the page's save toast implements.
   *
   ...
   *
   * ★ IT DOES NOT ASSUME EVERY CODED 422 IS A RATE-TABLE PROBLEM: an unrecognised code falls through
   * to the plain message path rather than being described as a bad ladder.
   */
  ...
  // ── The definition as the server expects it ───────────────────────────────────────────────

  /**
   * The rule definition as the server expects it.
   *
   * ★ ONE BUILDER, EVERY CALLER — saving on the Plans page, saving in the sandbox, and simulating on
   * either. If any of them assembled its own payload, the preview would eventually be describing a
   * slightly different rule from the one that gets stored, and nothing would report the divergence:
   * both requests would succeed. The guided tour's hand-written copy is the proof — its cap and floor
   * were sent in a shape the backend does not read.
   */
  buildDefinition(): AddRuleRequest {
    const v = this.form.getRawValue();
  ...
  private _buildModifier(v: ReturnType<typeof this.form.getRawValue>) {
    return {
      _schema: 1 as const,
      id: this.modifierId ?? crypto.randomUUID(),
...
/** The rule form's shape. A function so the type can be named before any instance exists. */
function buildRuleForm(fb: FormBuilder) {
  return fb.nonNullable.group({
    ...
    measurement: fb.nonNullable.group({
    ...
    rateTable: fb.nonNullable.group({
      type: [RateTableType.Flat as number, Validators.required],
      flatRate: [0.05],
      tiers: fb.array<FormGroup>([]),
      attainmentTiers: fb.array<FormGroup>([]),
      splitAtQuota: [false],
    }),
    hasTrigger: [false],
    trigger: fb.nonNullable.group({
      logicalOperator: [LogicalOperator.And as number],
      conditions: fb.array<FormGroup>([]),
    }),
    hasModifier: [false],
    modifier: fb.nonNullable.group({
      name: [''],
      type: [ModifierType.Accelerator as number],
      factor: [1.5],
    }),
    hasCap: [false],
    cap: fb.nonNullable.group({
      amount: [0, Validators.min(0)],
      scope: [CapScope.PerTransaction as number],
    }),
    hasFloor: [false],
    floor: fb.nonNullable.group({
      amount: [0, Validators.min(0)],
    }),
  });
}

type RuleForm = ReturnType<typeof buildRuleForm>;
